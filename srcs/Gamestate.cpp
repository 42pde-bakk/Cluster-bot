...
void Gamestate::spawn_new_children_drops(int chip_idx, int p_id) {
	if (chip_idx < 0)
		return;
	OffsetCoordinates	drop_coords;

	if (players[p_id].get_amount_left() <= 0)
		return ;
...
//		dprintf(2, "lets drop, col=%d, colour = %d\n", column, chip.colour + 10);
		int colour = child->players[p_id].get_chip(chip_idx);
		if (!child->drop_tile(column, colour + 10, &drop_coords)) {
			delete child;
			continue;
		}
//		dprintf(2, "drop was succesful\n");
		all_gamestates.emplace_back(child);
		child->action = Action(DROP, column - this->size + 1, colour);
		child->players[p_id].remove_chip();
...
//		dprintf(2, "PLACING chip: nb=%d, idx=%d, in column %d\n", chip.colour, chip_nb, column - this->size + 1);
		child->update_h(drop_coords, colour + 10);
...
void Gamestate::spawn_new_children_rotations(int player_id) {
	for (int i = 1; i < 6; i++) {
		...
		child->parent = this;
		// apply rotation
		child->get_player(player_id).remove_chip();
		child->get_player(player_id).remove_chip();
		// add to children
		children.emplace_back(child);
	}
...
void Gamestate::spawn_children_first(int player_id) {
	...
	this->children.clear();
	dprintf(2, "inventory: a=%d, b=%d\n", inv.a, inv.b);
	dprintf(2, "chips: amountleft=%d, colours = %d, %d\n", players[player_id].get_amount_left(), players[player_id].get_chip(0), players[player_id].get_chip(1));
	int idxs[2] = { inv.a - (2 * player_id), inv.b - (2 * player_id) };
	spawn_new_children_drops(idxs[0], player_id);
	if (inv.a != inv.b)
		spawn_new_children_drops(idxs[1], player_id);
